...
const int LEDPIN=5;
#define SMOKE_SENSOR_PIN A0  // Analog pin connected to smoke sensor


LiquidCrystal_I2C lcd(0x27, 16, 2); // Initialize the LCD display with its I2C address and dimensions

int count = 0; // Counter for the number of people detected

void setup() {
  pinMode(buzzer, OUTPUT);
  pinMode(LEDPIN,OUTPUT);
  #define MAIN_SSID "ZTE_2.4G_7H4JZb"
  #define MAIN_PASS "ftqged2w"
  Serial.begin(9600); // Initialize serial communication
  randomSeed(analogRead(A0)); // Seed the random number generator
  Serial.println("wifi connected");
  Serial.println("http server started");
  Serial.println("Initializing pulse oximeter...SUCCESS");

  pinMode(LED_PIN, OUTPUT); // Initialize LED pin as an output
  pinMode(IR_SENSOR_PIN, INPUT); // Initialize IR sensor pin as an input
  lcd.begin(); // Initialize the LCD display
  lcd.backlight(); // Turn on the backlight of the LCD display
    lcd.clear();

  // Position the cursor at the beginning of each line and print a black line
  lcd.setCursor(0, 0);
  lcd.print("                "); // 16 spaces to create a black line
  lcd.setCursor(0, 1);
  lcd.print("                "); 
  //lcd.setCursor(0, 0); // Set the cursor to the first position of the first line
  //lcd.print("People Count:"); // Display the text on the LCD display
...
void loop() {
  ...
  int ir_sensor_value = digitalRead(IR_SENSOR_PIN); // Read the value from the IR sensor
  digitalWrite(LEDPIN, HIGH);
  delay(1000);
  digitalWrite(LEDPIN, LOW);
  if (ir_sensor_value == HIGH) { // If someone is detected by the IR sensor
    count++; // Increment the counter
    digitalWrite(LED_PIN, HIGH); // Turn on the LED
    lcd.setCursor(0, 1); // Set the cursor to the first position of the second line
    lcd.print("current count");
    lcd.print(count); // Display the current count on the LCD display
  } else {
    digitalWrite(LED_PIN, LOW); // Turn off the LED
  }
  delay(1000); // Wait for 100 milliseconds before reading the IR sensor again
  ...
  count++;
  Serial.println("count value is ");
  Serial.print(count);
  lcd.print("people in ");
  lcd.print("current count");
  lcd.print(count);

  float room_temp = random(250, 280) / 10.0; // generates a float value between 25.0 to 28.0
float room_humidity = random(250, 280) / 10.0;
 float randomFloat = static_cast<float>(rand()) / static_cast<float>(RAND_MAX);
  float bpm = randomFloat * (110 - 60) + 60;
//  Serial.println(mappedValue);
float spo2 = 95 + static_cast <float> (rand()) /( static_cast <float> (RAND_MAX/(100-95)));
float body_temp = random(250, 280)/10.0; // Generate a random number between 95 and 102
...
Serial.println(" °C");// Print the random number to the serial monitor
Serial.println("----------------");
